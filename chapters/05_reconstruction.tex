...
We denote $x_S = x_A + x_B + \dots x_Y$, where $x_i$ corresponds to the emissions of sector $i \in \{A, B, \dots, Y\}$, as the sum of all sectors.
...
\begin{align}
    y &= A_S (x_A + x_B + \dots + x_Y) + \epsilon \\
    y &= A_S x_A + A_S x_B + \dots + A_S x_Y + \epsilon \\
\end{align}
Rewriting this as a matrix multiplication yields the following
\begin{equation}
    y = \begin{bmatrix} A_S & A_S & \cdots & A_S \end{bmatrix}\begin{bmatrix} x_A \\ x_B \\ \vdots \\x_Y \end{bmatrix} + \epsilon
\end{equation}
